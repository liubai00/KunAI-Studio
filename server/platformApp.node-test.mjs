...
test('self-contained platform charges image generations from CNY balance and keeps payment idempotent', async (t) => {
  ...
  assert.equal(status.data.register_enabled, true)
  assert.equal(status.data.password_register_enabled, true)
...
test('Agent relay replays settled calls before model and reserve policy checks', async (t) => {
  ...
  assert.equal(db.getUserById(adminId).balanceMicros, 999993)
  assert.equal(db.getUserById(adminId).usedMicros, 7)
  assert.equal(db.listLedger(adminId).filter((entry) => entry.kind === 'agent_charge').length, 1)
