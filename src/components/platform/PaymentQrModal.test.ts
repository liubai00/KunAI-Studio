...
describe('payment QR polling', () => {
  ...
  it('renders an accessible QR payment flow without browser dialogs', () => {
    ...
    expect(source).toContain('useCloseOnEscape(true, props.onClose)')
    expect(source).toContain('useModalFocus(true, dialogRef, props.returnFocusRef)')
    expect(source).toContain('usePreventBackgroundScroll(true, dialogRef)')
