      ...
      >
        <div className="flex h-14 items-center justify-end px-4 md:hidden">
          <button
            ...
            className="grid place-items-center h-[34px] w-[34px] rounded-[10px] text-ink-3 hover:bg-surface2 hover:text-ink transition-colors"
            aria-label="关闭"
          >
        ...
        {/* 左侧：图片 */}
        <div className="md:w-[45%] w-full h-64 md:h-auto bg-surface2 md:border-r border-line relative flex items-center justify-center flex-shrink-0 min-h-[16rem]">
          ...
          {task.status === 'done' && outputLen > 0 && currentOutputPreviewSrc && (
            <>
              <img
                ...
                className="saveable-image max-w-[calc(100%-2rem)] max-h-[calc(100%-2rem)] object-contain cursor-pointer rounded-xl shadow-[0_12px_34px_-12px_rgba(0,0,0,0.45)]"
                onLoad={(e) => {
            ...
            <>
              ...
              {task.status === 'running' && streamPreviewLen > 0 && (
                <>
                  ...
                  {streamPreviewLoaded && (
                    <span className="absolute top-4 right-4 flex items-center gap-1 rounded bg-info px-2 py-0.5 text-xs font-medium text-white backdrop-blur-sm">
                      流式预览
                    </span>
                  )}
                  {streamPreviewLen > 1 && (
              ...
              <div className="mt-3 flex items-center justify-center gap-2">
                <div className="relative group">
                  <button
                    ...
                    className="inline-flex items-center justify-center rounded-full border border-red-500/30 px-3 py-1.5 text-red-500 transition hover:bg-red-500/10"
                    aria-label="复制完整报错"
                  >
                ...
                {task.rawResponsePayload && (
                  <div className="relative group">
                    <button
                      ...
                      className="inline-flex items-center justify-center rounded-full border border-info/30 px-3 py-1.5 text-info transition hover:bg-info/10"
                      aria-label="查看原始响应"
                    >
                ...
                {task.rawImageUrls && task.rawImageUrls.length > 0 && (
                  <div className="relative group">
                    <button
                      ...
                      className="inline-flex items-center justify-center rounded-full border border-emerald-500/30 px-3 py-1.5 text-emerald-600 transition hover:bg-emerald-500/10 dark:text-emerald-400"
                      aria-label="复制图片链接"
                    >
                ...
                {streamPartialImageIds.length > 0 && (
                  <div className="relative group">
                    <button
                      ...
                      className="inline-flex items-center justify-center rounded-full border border-accent/30 px-3 py-1.5 text-accent-ink transition hover:bg-accent-soft"
                      aria-label="下载中间步骤图"
                    >
                      <DownloadIcon className="h-4 w-4" />
                    </button>
                ...
                <div className="relative group">
                  <button
                    ...
                    className="inline-flex items-center justify-center rounded-full border border-accent/40 px-3 py-1.5 text-accent-ink transition hover:bg-accent-soft"
                    aria-label="重试任务"
                  >
        ...
        {/* 右侧：信息 */}
        <div className="md:flex-1 w-full p-6 overflow-y-auto overscroll-contain flex flex-col">
          <button
            onClick={() => setDetailTaskId(null)}
            className="absolute top-3 right-3 hidden md:grid place-items-center h-[34px] w-[34px] rounded-[10px] hover:bg-surface2 hover:text-ink transition-colors text-ink-2 z-10"
            aria-label="关闭"
          >
          ...
          <div data-selectable-text className="flex-1">
            ...
            {showPendingPrompt ? (
              <div className="mb-[22px] leading-relaxed">
                <p className="text-[15px] text-ink">正在生成……</p>
                <p className="mt-1 text-xs text-ink-3">输入内容将在响应完成时接收</p>
              </div>
            ) : (
                      ...
                      {allInputImageIds.map((imgId) => {
                        ...
                        return (
                          <div key={imgId} className="relative group inline-block">
                            <div
                              className={`relative w-16 h-16 rounded-[10px] overflow-hidden border cursor-pointer hover:opacity-80 transition ${
                                isMaskTarget ? 'border-accent border-2 shadow-card' : 'border-line'
                              }`}
                            ...
                            >
                              ...
                              {isMaskTarget && (
                                <span className="absolute left-1 top-1 rounded bg-accent/90 px-1.5 py-0.5 text-[8px] leading-none text-white font-bold tracking-wider backdrop-blur-sm z-10 pointer-events-none">
                                  MASK
                                </span>
                              )}
                            </div>
            ...
            {/* 参数 */}
            <h3 className="text-xs font-semibold text-ink-3 tracking-[0.03em] mb-2.5">
              参数配置
            </h3>
            {showSourceInfo && (
              <div className="mb-3 min-w-0 overflow-hidden rounded-xl border border-line bg-surface2 px-3.5 py-3 text-xs">
                <span className="text-[11px] font-medium text-ink-3">来源</span>
                <div className="mt-1 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2 text-[13.5px]">
                  <span className="font-semibold text-ink">{taskProviderName}</span>
                  <span className="text-ink-2"> · {taskProfileName} · {taskModel}</span>
                </div>
              </div>
            )}
            <div className="grid grid-cols-2 gap-3 text-xs mb-5 min-w-0">
              <div className="rounded-xl border border-line bg-surface2 px-3.5 py-3 min-w-0 overflow-hidden">
                <span className="text-[11px] font-medium text-ink-3">尺寸</span>
                <br />
                <div className="mt-1 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2 font-mono text-[13.5px]">
                  <DetailParamValue task={task} paramKey="size" className="font-medium" actualParams={currentActualParams} />
                </div>
              </div>
              <div className="rounded-xl border border-line bg-surface2 px-3.5 py-3 min-w-0 overflow-hidden">
                <span className="text-[11px] font-medium text-ink-3">质量</span>
                <br />
                <div className="mt-1 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2 font-mono text-[13.5px]">
              ...
              <div className="rounded-xl border border-line bg-surface2 px-3.5 py-3 min-w-0 overflow-hidden">
                <span className="text-[11px] font-medium text-ink-3">格式</span>
                <br />
                <div className="mt-1 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2 font-mono text-[13.5px]">
              ...
              {isPngOutput ? (
                <div className="rounded-xl border border-line bg-surface2 px-3.5 py-3 min-w-0 overflow-hidden">
                  <span className="text-[11px] font-medium text-ink-3">透明背景</span>
                  <br />
                  <div className="mt-1 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2 font-mono text-[13.5px]">
                    <span className="font-medium text-ink">{transparentOutputText}</span>
                    {currentTransparentOutputFailed && (
                      <span className="ml-1.5 rounded bg-red-500/10 px-1 py-0.5 text-[10px] font-medium uppercase leading-none text-red-600 dark:text-red-400">
                        failed
                      </span>
                    )}
                  </div>
                ...
                <div className="rounded-xl border border-line bg-surface2 px-3.5 py-3 min-w-0 overflow-hidden">
                  <span className="text-[11px] font-medium text-ink-3">压缩率</span>
                  <br />
                  <div className="mt-1 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2 font-mono text-[13.5px]">
                    <span className="font-medium text-ink">{outputCompressionText}</span>
                  </div>
                </div>
              )}
              <div className="rounded-xl border border-line bg-surface2 px-3.5 py-3 min-w-0 overflow-hidden">
                <span className="text-[11px] font-medium text-ink-3">审核</span>
                <br />
                <div className="mt-1 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2 font-mono text-[13.5px]">
              ...
              {!isAgentTask && (
                <div className="rounded-xl border border-line bg-surface2 px-3.5 py-3 min-w-0 overflow-hidden">
                  <span className="text-[11px] font-medium text-ink-3">数量</span>
                  <br />
                  <div className="mt-1 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2 font-mono text-[13.5px]">
            ...
            {/* 时间 */}
            <div className="text-[12.5px] text-ink-3 mb-5">
          ...
          {/* 操作按钮 */}
          <div className="grid grid-cols-4 sm:flex sm:items-center gap-2.5 pt-4 border-t border-line">
            <button
              onClick={handleReuse}
              className="col-span-2 sm:flex-1 h-[42px] flex items-center justify-center gap-2 px-4 rounded-xl bg-[linear-gradient(150deg,var(--accent),#e07a1f)] text-white shadow-[0_8px_18px_-6px_var(--accent-glow)] hover:-translate-y-px transition text-[13px] font-semibold whitespace-nowrap"
            >
            ...
            <button
              onClick={handleEdit}
              disabled={!outputLen}
              className="col-span-2 sm:flex-1 h-[42px] flex items-center justify-center gap-2 px-4 rounded-xl bg-surface2 text-ink border border-line hover:border-line2 disabled:opacity-40 disabled:cursor-not-allowed transition text-[13px] font-semibold whitespace-nowrap"
            >
            ...
            <button
              ...
              className="col-span-3 sm:flex-1 h-[42px] flex items-center justify-center gap-2 px-4 rounded-xl bg-transparent text-red-500 border border-red-500/30 hover:bg-red-500/10 transition text-[13px] font-semibold whitespace-nowrap"
            >
            ...
            <button
              ...
              className={`col-span-1 sm:flex-none sm:w-[42px] w-full h-[42px] flex items-center justify-center rounded-xl border transition-colors ${
                task.isFavorite
                  ? 'border-accent/40 bg-accent-soft text-accent'
                  : 'border-line text-ink-3 hover:bg-surface2 hover:text-ink'
              }`}
      ...
      {showRawUrlsModal && rawImageUrls.length > 0 && (
        <GlobalModal layer="dialog" onClose={() => setShowRawUrlsModal(false)} className="p-4 sm:p-6">
          <div ref={rawUrlsModalRef} role="dialog" aria-modal="true" aria-labelledby="raw-image-urls-title" tabIndex={-1} className="relative z-10 flex w-full max-w-2xl max-h-[90vh] flex-col overflow-hidden rounded-2xl bg-surface border border-line2 shadow-lift">
            <div className="flex items-center justify-between border-b border-line px-5 py-4 shrink-0">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg bg-surface2 text-ink-2 border border-line hover:text-ink transition-colors text-xs font-medium"
                >
            ...
            <div className="flex-1 min-h-0 overflow-y-auto p-3 sm:p-5 bg-surface2 overscroll-contain">
              <div className="space-y-2.5">
                {rawImageUrls.map((url, i) => (
                  <div key={i} className="group flex items-center gap-3 p-3 sm:p-4 rounded-xl bg-surface border border-line shadow-card hover:shadow-lift transition-all">
                    <div className="flex-1 min-w-0 flex flex-col gap-1">
                      <div className="text-xs font-medium text-ink-3">
                        图片 {i + 1}
                      </div>
                      <div className="text-sm text-ink truncate select-text" title={url}>
                        {url}
                      </div>
                    </div>
                    <button
                      ...
                      className="flex-shrink-0 p-2 sm:px-3 sm:py-1.5 flex items-center justify-center gap-1.5 rounded-lg bg-surface2 text-ink-2 hover:text-ink transition-colors text-xs font-medium border border-line"
                      title="复制链接"
                    >
      ...
      {showRawResponseModal && task?.rawResponsePayload && (
        <GlobalModal layer="dialog" onClose={() => setShowRawResponseModal(false)} className="p-4 sm:p-6">
          ...
          >
            ...
            <div className="flex-1 min-h-0 overflow-y-auto p-5 bg-surface2 overscroll-contain">
              <pre data-selectable-text className="text-[11px] sm:text-xs text-ink-2 font-mono whitespace-pre-wrap break-all select-text">
