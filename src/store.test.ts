...
describe('interrupted OpenAI running tasks', () => {
  it('marks legacy and OpenAI running tasks as interrupted', () => {
    ...
    const platformRunning = task({
      ...
      finishedAt: null,
      elapsed: null,
      platformRequestIds: ['platform-request-0001'],
      platformRecoverable: true,
    })
    const doneTask = task({ id: 'done-task', apiProvider: 'openai', status: 'done' })

    const result = markInterruptedOpenAIRunningTasks([legacyRunning, openAIRunning, falRunning, customAsyncRunning, platformRunning, doneTask], now)
    ...
    expect(result.tasks.find((item) => item.id === 'platform-running')).toEqual(platformRunning)
