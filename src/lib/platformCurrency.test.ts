...
import { formatPlatformLedgerAmount, formatPlatformPrice, formatPlatformQuota, getPlatformGenerationPriceMicros, getPlatformImagePrice } from './platformCurrency'
...
describe('platform currency', () => {
  ...
  it('selects the image price for the active resolution tier', () => {
    const status = {
      image_studio: {
        ...
        image_prices: { '1k': 0.15, '2k': 0.2, '4k': 0.35 },
    ...
    expect(getPlatformImagePrice(status, '4K')).toBe(0.35)
  })
  ...
  it('formats sub-cent ledger charges without producing negative zero', () => {
    const status = { quota_display_type: 'CNY', quota_per_unit: 1000000 }
    expect(formatPlatformLedgerAmount(-7, status)).toBe('-<¥0.01')
    expect(formatPlatformLedgerAmount(7, status)).toBe('+<¥0.01')
    expect(formatPlatformLedgerAmount(0, status)).toBe('¥0.00')
    expect(formatPlatformQuota(-7, status)).toBe('¥0.00')
  })
})
